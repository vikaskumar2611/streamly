...
const getCurrentUser = asyncHandler(async (req, res) => {
    return res
        .status(200)
        .json(
            new ApiResponse(200, req.user, "current user fetched seccessfully"),
        );
});
...
const updateUserAvatar = asyncHandler(async (req, res) => {
    ...
    const deleteResult = await deleteOnCloudinary(oldimageurl);
    if (deleteResult?.result !== "ok") {
        console.warn(`Error deleting old avatar file -,${oldimageurl}`);
    }

    // Since the primary operation succeeded, you shouldn't fail the entire request. Instead:
    // await logOrphanedFile(oldImageUrl);
    ...
    return res
        ...
        .json(
            new ApiResponse(
                200,
                user,
                "cover Image image updated successfully",
            ),
        );
});
